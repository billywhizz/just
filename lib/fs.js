...
const st = {} // single instance to hold file stats. we should create a class for this but it should get optimised after a number of uses anyway as interface never changes
...
function readFileBytes (path, flags = fs.O_RDONLY) {
  ...
  let off = 0
  let len = net.read(fd, buf, chunkSize, off)
  while (len > 0) {
    off += len
    len = net.read(fd, buf, chunkSize, off)
  }
  off += len
  r = net.close(fd)
  if (len < 0) throw new Error(`Error Reading File: ${sys.errno()}`)
  if (off < size) throw new Error(`Size Mismatch: size: ${size}, read: ${off}`)
  if (r < 0) throw new Error(`Error Closing File: ${sys.errno()}`)
  return buf
}

function readFile (path, buf = new ArrayBuffer(4096), flags = fs.O_RDONLY) {
  const chunks = []
  const fd = fs.open(path, flags)
...
function writeFile (path, buf, flags = fs.O_WRONLY | fs.O_CREAT | fs.O_TRUNC) {
  ...
  if (bytes < 0) throw new Error(`Error Writing to File: ${sys.errno()} (${sys.strerror(sys.errno())})`)
...
module.exports = { readFile, readFileBytes, writeFile, getStats, fileType, checkMode, checkFlag }
