 ...
 private:
  ...
  void Send(const v8_inspector::StringView& string) {
    ...
    if (callback->IsFunction()) {
      ...
      Local<Function>::Cast(callback)->Call(context, Undefined(isolate_), 1, args).ToLocalChecked();
    }
  }
 ...
 public:
  ...
  void runMessageLoopOnPause(int context_group_id) override {
    ...
    if (callback->IsFunction()) {
      v8::TryCatch try_catch(isolate_);
      Local<Value> args[] = {};
      Local<Function>::Cast(callback)->Call(context, Undefined(isolate_), 0, args).ToLocalChecked();
    }
  }
